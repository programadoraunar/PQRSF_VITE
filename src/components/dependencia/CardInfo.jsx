...
function CardInfo(props) {
	...
	switch (solicitud) {
		case 'Asignadas':
			...
			title = 'Numero de PQRSF Por Responder';
			icon = <RiProgress1Line className='w-20 h-16' />;
			status = 'bg-yellow-500/10 text-yellow-500';
			description =
				'Actualmete tienes este numero de Solicitudes por responder!';
		...
		case 'Cerradas':
			...
			description = 'Solicitudes que has cerrado';
		...
		case 'total':
			...
			description = 'Total de solicitudes que ten han llegado';
		...
		case 'promedio':
			borderColor = 'border-t-4 border-red-500';
			title = 'Total de Solicitudes';
			icon = <RiListView className='w-16 h-16' />;
	...
	return (
		...
		>
			<figure className={`${status} min-w-[80px]`}>{icon}</figure>
